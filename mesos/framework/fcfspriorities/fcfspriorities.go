...
func Start(frameworkConf data.FrameworkConf) {
	taskId = 0
	log.Println("[fcfspriorities] ----------> SCHEDULER LOADING < ---------- [fcfspriorities]")
	...
	frameworkInfoFile = configuration.FrameworksConf + "/fcfspriorities/" + frameworkConf.Framework.FrameworkInfo
	stateFile = configuration.FrameworksConf + "/fcfspriorities/" + frameworkConf.Framework.State

	log.Printf("[fcfspriorities] Try to load previous FrameworkInfo from %s", frameworkInfoFile)
	...
	if err == nil {
		...
		if err != nil {
			log.Printf("[fcfspriorities] Error %v. Please delete %s and restart", err, frameworkInfoFile)
		}
	} else {
		log.Printf("[fcfspriorities] Fallback to defaults due to error [%v]", err)
	...
	if errorsub == nil {
		...
		log.Printf("[fcfspriorities] Problems to subreibe against mesos")
		...
		if priority > -1 && tasklen > 0 {
			...
			for i := 0; i < len(selectedServer) && i < t.Instances; i++ {
				...
				if t.Name == "eidasServer" {
					...
					localCMD = localCMD + " " + "Prio_" + strconv.Itoa(t.QAL) + "_" + subcrition.Host + "_" + strconv.Itoa(Portaux) + " http://" + subcrition.Host + " " + subcrition.Port + " " + sendResults + " " + strconv.FormatFloat(subcrition.Lambda, 'E', -1, 64) + " " + strconv.Itoa(subcrition.Nclients) + " " + subcrition.Timestamp.Format("15:04:05") + " " + sendTimes + " " + t.Time
				}

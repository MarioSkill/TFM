...
import (
	...
	loadbalancer "gitlab.com/marioskill/loadbalancer/prioridades"
...
func Start(frameworkConf data.FrameworkConf) {
	taskId = 0
	log.Println("[FCFS-D-PRIORIDADES] ----------> SCHEDULER LOADING < ---------- [FCFS-D-PRIORIDADES]")
	...
	frameworkInfoFile = configuration.FrameworksConf + "/fcfsd-priorities/" + frameworkConf.Framework.FrameworkInfo
	stateFile = configuration.FrameworksConf + "/fcfsd-priorities/" + frameworkConf.Framework.State

	log.Printf("[FCFS-D-PRIORIDADES] Try to load previous FrameworkInfo from %s", frameworkInfoFile)
	...
	if err == nil {
		...
		if err != nil {
			log.Printf("[FCFS-D-PRIORIDADES] Error %v. Please delete %s and restart", err, frameworkInfoFile)
		}
	} else {
		log.Printf("[FCFS-D-PRIORIDADES] Fallback to defaults due to error [%v]", err)
	...
	if errorsub == nil {
		...
		log.Printf("[FCFS-D-PRIORIDADES] Problems to subreibe against mesos")
	...
	if posclientHost != -1 {
		...
		for ; i < len(selectedServer) && t.Instances > i; i++ {
			...
			*/
			...
			localCMD = localCMD + " " + "Prio_" + strconv.Itoa(t.QAL) + subcrition.Host + "_" + strconv.Itoa(Portaux) + " http://" + subcrition.Host + " " + subcrition.Port + " " + sendResults + " " + strconv.FormatFloat(subcrition.Lambda, 'E', -1, 64) + " " + strconv.Itoa(subcrition.Nclients) + " " + subcrition.Timestamp.Format("15:04:05") + " " + sendTimes + " " + t.Time
			agent := data.AID
			TRUE := true
			aux := &rpc.TaskInfo{
				Name:      &nameAPP,

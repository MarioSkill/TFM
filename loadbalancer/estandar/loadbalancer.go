...
func SubClient(id string, host string, ip string, version string, p int, qal int) ServerInformation {
	mTick.Lock()
	...
	if nClients > maxClientsSupported {
		...
		data.Set("testConf", "eIDAS_Server_lb.json")
		fmt.Printf("LB")
		switch global.GetSchedulerName() {
		case "fcfs":
			sendResult("http://localhost:"+configuration.GUI_port+"/schedulers/fcfs/balancer", data)
		case "fcfs-d":
			fmt.Printf("LB2")
			sendResult("http://localhost:"+configuration.GUI_port+"/schedulers/fcfs-d/balancer", data)
		case "priorities":
	...
	if nServers > 0 {
		...
		aux := ServerInformation{ID: sInfo.ID, Host: sInfo.Host, Port: sInfo.Port, Lambda: 0.1, Timestamp: time.Now(), Nclients: clientsPerSubcription}
		mTick.Unlock()
		return aux
	} else {
		mTick.Unlock()
